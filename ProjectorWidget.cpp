...
cv::Mat ProjectorWidget::CreateLinePattern()
{
  ...
  if( reverse == true )
    {
    map_pattern.emplace( cv::Vec3b( pattern.at( size - 2 ), pattern.at( size - 1 ), 99 ), 1*step );
    map_pattern.emplace( cv::Vec3b( pattern.at( size - 1 ), 99, 99 ), 0*step );
    std::cout << " i : " << size - 2 << " 1*step : " << 1*step << " " << cv::Vec3b( pattern.at( size - 2 ), pattern.at( size - 1 ), 99 ) << std::endl;
    std::cout << " i : " << size - 1 << " 0*step : " << 2*step << " " << cv::Vec3b( pattern.at( size - 1 ), 99, 99 ) << std::endl;
    }
  else
    {
